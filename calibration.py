...
def interpolate_to_frequencies(rate, ori_time_list, pos_list, obj_start, obj_end):
	#interpolate to make signals from motion capture to robot at the same frequency 
    frequency = rate
    time_list = []
    val_list = []
    obj_list = []
    for time_stamp in ori_time_list:
        time_list.append(time_stamp.to_sec())
    for val in pos_list:
    	val_list.append(val)

    time_interval_obj = np.arange(obj_start, obj_end, float(1) / frequency)
...
def save_as_pickle(transformation_matrix, pkl_file_name):
	#write the calculated trans_matrix into a .pkl file
	pkl_file_obj = open(pkl_file_name, 'wb')
	pickle.dump(transformation_matrix, pkl_file_obj, -1)
...
def do_calib(rigidBodyPos, pos_rob, joint_angles, time_stamp_rob, test_plot=False):
	...
	obj_end_stamp = end_stamp.to_sec()
	obj_start_stamp = start_stamp.to_sec()
	...
	it_pos_rob_x = interpolate_to_frequencies(
								RATE, time_stamp_rob, pos_rob_x, obj_start_stamp, obj_end_stamp)
	it_pos_rob_y = interpolate_to_frequencies(
								RATE, time_stamp_rob, pos_rob_y, obj_start_stamp, obj_end_stamp)
	it_pos_rob_z = interpolate_to_frequencies(
								RATE, time_stamp_rob, pos_rob_z, obj_start_stamp, obj_end_stamp)

	it_pos_mp_x = interpolate_to_frequencies(
								RATE, moCap_time_stamp, moCap_pos_x, obj_start_stamp, obj_end_stamp)
	it_pos_mp_y = interpolate_to_frequencies(
								RATE, moCap_time_stamp, moCap_pos_y, obj_start_stamp, obj_end_stamp)
	it_pos_mp_z = interpolate_to_frequencies(
								RATE, moCap_time_stamp, moCap_pos_z, obj_start_stamp, obj_end_stamp)
	for i in range(len(joint_angles)):
		interpolated_joint_angle_table[i] = interpolate_to_frequencies(
								RATE, time_stamp_rob, joint_angles[i], obj_start_stamp, obj_end_stamp)
	...
	#save_as_pickle(M_mocap_robot, "pkl_files/moCap_robot_calib.pkl")
...
if __name__ == '__main__':
	...
	fileName = "calib_robot_log/calibration_320.txt"
	...
	moCap_time_stamp, moCap_pos_x, moCap_pos_y, moCap_pos_z = parsePosTables(rigidBodyPos)
	matrix_after_transform, M_mocap_robot, rob_endEffector_pos = do_calib(
		rigidBodyPos, pos_rob, reshaped_joint_angles, time_stamp_rob,
		test_plot=False)
	rob_EE_pos_x, rob_EE_pos_y, rob_EE_pos_z = parsePosTables(rob_endEffector_pos)
	...
	#robot trajectory
	ax.plot(pos_rob_x, pos_rob_y, pos_rob_z, c='b', linewidth=1.5)
	#moCap transformed data (this should be aligned with the robot data)
...
#	ax.plot(moCap_pos_x, moCap_pos_y, moCap_pos_z, c='g', linewidth=1)
	fig_2 = plt.figure(2)
	plt.plot([i for i in range(len(pos_rob_x))], pos_rob_x, c='r')
	plt.plot([i for i in range(len(pos_rob_x))], pos_rob_y, c='g')
	plt.plot([i for i in range(len(pos_rob_x))], pos_rob_z, c='b')
	plt.show()