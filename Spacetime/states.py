...
# utility that I don't know where else to put...
# compute the derivative of the signal, as its done in spacetime
def derivSignal(sig):
    v = N.empty(len(sig))
    v[0] = sig[1]-sig[0]
...
def d2Signal(sig):
    v = N.empty(len(sig))
    v[0] = sig[2]-2*sig[1]+sig[0]
    for i in range(1,len(sig)-1):
        v[i] = sig[i+1]-2*sig[i]+sig[i-1]
    v[-1] = v[-2]
    return v
